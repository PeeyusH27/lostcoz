  ...
  return (
    ...
    <section id="events" ref={section} className="relative z-[2] scroll-mt-20 overflow-hidden py-24 md:h-svh md:py-0">
      <div ref={track} className="flex snap-x snap-mandatory gap-6 overflow-x-auto px-gutter scroll-px-gutter no-scrollbar md:h-svh md:snap-none md:items-center md:gap-8 md:overflow-visible md:pr-[10vw]">
        <div data-panel className="flex w-[82vw] shrink-0 snap-start flex-col justify-center md:w-[42vw] md:min-w-[26rem]">
        ...
        {site.events.formats.map((f) => (
          <div key={f.num} data-panel className="w-[82vw] shrink-0 snap-start md:w-[34rem]">
            <Tilt max={6} className="rounded-2xl">
              <article className="relative flex h-[30rem] flex-col justify-between overflow-hidden rounded-2xl bg-surface-1 p-8 hairline md:h-[34rem] md:p-10" data-cursor="link">
                ...
                <div className="flex items-start justify-between">
                  ...
                  <span className="rounded-pill border border-line px-3 py-1.5 font-body text-caption uppercase tracking-widest text-fg-muted">{f.tag}</span>
                </div>
                <div>
