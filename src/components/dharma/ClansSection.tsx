  ...
  return (
    <section ref={ref} className="relative px-gutter py-section">
      <div className="relative z-[2] mx-auto max-w-content">
        ...
        <div className="mt-20 grid gap-10 sm:grid-cols-3 sm:gap-6">
          {clans.map((c, i) => (
            <Reveal key={c.slug} y={40} delay={i * 0.1}>
              <div className="mx-auto w-[14rem] sm:w-full sm:max-w-[16rem] [perspective:1200px]">
                <Tilt max={10} className="card-ratio rounded-card">
                  <div data-flip className="card3d relative size-full rounded-card shadow-card-lift" data-cursor="text" data-cursor-text={c.name}>
                    <div className="face front"><Image src={c.image.src} width={c.image.w} height={c.image.h} alt={c.name} sizes="16rem" className="size-full object-cover" draggable={false} /></div>
                    <div className="face back"><Image src={backs.clan.small} width={540} height={840} alt="" sizes="16rem" className="size-full object-cover" draggable={false} /></div>
                  </div>
                </Tilt>
              </div>
