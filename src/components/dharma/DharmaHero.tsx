...
export default function DharmaHero() {
  ...
  const fan = [
    { ...backs.ability, rot: -16, x: "-58%", z: 1 },
    { ...backs.clan, rot: 0, x: "0%", z: 3 },
    { ...backs.eliminated, rot: 16, x: "58%", z: 2 },
  ];

  return (
    <section ref={ref} className="relative min-h-[100svh] overflow-hidden px-gutter pb-16 pt-36 grain">
      ...
      <div className="relative z-[2] mx-auto grid max-w-wide items-center gap-14 lg:grid-cols-[1.25fr_1fr]">
        <div data-h-copy>
          ...
          <dl className="mt-14 grid grid-cols-3 gap-y-8 sm:grid-cols-5">
            {facts.map((f) => (
        ...
        <div data-h-cards className="relative mx-auto flex h-[26rem] w-full max-w-[36rem] items-center justify-center sm:h-[32rem]">
          <div aria-hidden="true" className="absolute size-[26rem] rounded-full bg-dharma-gold-500/20 blur-[100px] animate-pulse-glow" />
          {fan.map((c) => (
            <div key={c.title} data-h-card className="absolute w-[10rem] sm:w-[13rem] lg:w-[14rem]" style={{ transform: `translateX(${c.x}) rotate(${c.rot}deg)`, zIndex: c.z, transformOrigin: "50% 130%" }}>
              <Tilt max={16} className="card-ratio rounded-card">
                <div className="size-full overflow-hidden rounded-card shadow-card-lift" data-cursor="text" data-cursor-text="Deal">
                  <Image src={c.src} width={c.w} height={c.h} alt={c.title} sizes="14rem" priority className="size-full object-cover" draggable={false} />
                </div>
              </Tilt>
            </div>
          ))}
